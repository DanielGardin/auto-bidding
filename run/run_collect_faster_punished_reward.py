...
dummy_strategy = get_strategy("AlphaPunisherStrategy", actor=dummy_actor, budget=1e6, cpa=1e6, category=0)
...
def collect_rl_data(
    ...
    save_path = get_root_path() / "data/traffic/new_gen_data"
    index       = []
    states      = []
    actions     = []
    rewards     = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("--strategy", type=str, default="AlphaPunisherStrategy")
    parser.add_argument("--filename", type=str, default="punished_reward_rl_data")
    ...
    for data in ["old", "new"]:
        ...
        df.append(pd.read_parquet(get_root_path() / "data/traffic/new_gen_data" / filename))
    ...
    with open(get_root_path() / "data/traffic/new_gen_data" / filename_pkl, "wb") as f:
        pkl.dump(state_norm, f)

    df.to_parquet(get_root_path() / "data/traffic/new_gen_data" / filename)